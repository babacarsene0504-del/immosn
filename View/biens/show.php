...
<body>
    ...
    <main class="max-w-5xl mx-auto mt-8 p-6 grid grid-cols-3 gap-8">

        <div class="col-span-2">
            <div class="h-64 bg-orange-50 rounded mb-4"></div>

            <div class="flex justify-between items-start">
                <div>
                    <span class="text-xs px-2 py-1 rounded bg-emerald-100 text-emerald-800">
                        <?= $bien['type_offre'] === 'location' ? 'Location' : 'Vente' ?>
                    </span>
                    <h1 class="text-2xl font-bold mt-2"><?= htmlspecialchars($bien['titre'], ENT_QUOTES, 'UTF-8') ?></h1>
                    <p class="text-gray-500"><?= htmlspecialchars($bien['ville_nom'] ?? '', ENT_QUOTES, 'UTF-8') ?></p>
                </div>
                <p class="text-2xl font-bold text-orange-600 whitespace-nowrap">
                ...
                <div class="grid grid-cols-3 gap-4">
                    <?php foreach ($similaires as $s): ?>
                        <a href="/biens/<?= htmlspecialchars($s['id'], ENT_QUOTES, 'UTF-8') ?>" class="border rounded overflow-hidden block">
                            <div class="h-24 bg-orange-50"></div>
                            <div class="p-2">
                                <p class="text-sm font-medium truncate"><?= htmlspecialchars($s['titre'], ENT_QUOTES, 'UTF-8') ?></p>
                                <p class="text-orange-600 text-sm font-bold"><?= number_format($s['prix'], 0, ',', ' ') ?> FCFA</p>
                            </div>
                        </a>
                    <?php endforeach; ?>
                </div>
        ...
        <div class="col-span-1 space-y-4">
            ...
            <?php if ($proprietaire): ?>
                <div class="border rounded p-4">
                    ...
                    <button class="w-full bg-orange-600 text-white py-2 rounded text-sm mb-2">
                        Contacter le propriétaire
                    </button>
                    <button class="w-full border py-2 rounded text-sm">
                        Ajouter aux favoris
                    </button>
                </div>
            <?php endif; ?>
